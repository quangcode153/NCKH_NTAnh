...
ROBERTA_BASE_CHINESE = 'chinese-roberta-wwm-ext' 
ROBERTA_LARGE_CHINESE = 'chinese-roberta-wwm-ext-large' 
DEBERTA_LARGE_CHINESE = 'deberta-chinese-large' 
ELECTRA_SMALL_CHINESE = 'chinese-electra-180g-small' 
ELECTRA_BASE_CHINESE  = 'chinese-electra-180g-base' 
ELECTRA_LARGE_CHINESE = 'chinese-electra-180g-large' 
XLNET_BASE_CHINESE = 'chinese-xlnet-base' 
MACBERT_BASE_CHINESE = 'chinese-macbert-base' 
MACBERT_LARGE_CHINESE = 'chinese-macbert-large' 
PERT_BASE_CHINESE = 'chinese-pert-base' 
PERT_LARGE_CHINESE = 'chinese-pert-large' 
LERT_SMALL_CHINESE = 'chinese-lert-small' 
LERT_BASE_CHINESE  = 'chinese-lert-base' 
LERT_LARGE_CHINESE = 'chinese-lert-large' 
GPT2_CHINESE = 'gpt2-chinese-cluecorpussmall' 
CLIP_CHINESE = 'taiyi-clip-roberta-chinese' 
WENZHONG_GPT2_CHINESE = 'wenzhong2-gpt2-chinese' 
ALBERT_TINY_CHINESE = 'albert_chinese_tiny' 
ALBERT_SMALL_CHINESE = 'albert_chinese_small' 
SIMBERT_BASE_CHINESE = 'simbert-base-chinese' 
    ...
    for idx, row in df.iterrows():
        ...
        if pd.isna(sentence) == False and len(sentence) > 0:
            ...
            with torch.no_grad():
                ...
                outputs = torch.stack(outputs)[layer_ids].sum(dim=0) 
    ...
    for idx, row in df.iterrows():
        ...
        for word in words:
            if word in ['.', '!', '?']:
                ...
                if 'uncased' in model_name or 'albert' in model_name or 'electra' in model_name: 
            ...
            with torch.no_grad():
                ...
                for i in range(real_batch_size): 
                    if 'xlnet' in model_name:
                        input_id = input_ids[i, -lens[i]:-2] 
                        output = outputs[i, -lens[i]:-2]  
                    else: 
                        input_id = input_ids[i, 1:(lens[i] - 1)] 
                    ...
                    if n_tokens == n_words: 
                        ...
                        for j, token_id in enumerate(input_id):
                            ...
                            if token == current_word or token == '[UNK]': 
                                ...
                                if  word == current_word: 
                                    if combine_type == 'sum':
                                        word_embedding = np.sum(np.row_stack(word_embedding), axis=0) 
                                    elif combine_type == 'mean':
                                        word_embedding = np.mean(np.row_stack(word_embedding), axis=0) 
